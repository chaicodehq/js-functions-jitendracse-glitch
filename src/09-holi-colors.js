...
export function adjustBrightness(color, factor) {
  ...
  const newR = Math.max(0, Math.min(255, Math.round(color.r * factor)));
  const newG = Math.max(0, Math.min(255, Math.round(color.g * factor)));
  const newB = Math.max(0, Math.min(255, Math.round(color.b * factor)));
